# tabs/tab_riduzione.py
"""Tab Riduzione - costruzione UI."""
...
def build(app):
    """Costruisce la tab Riduzione nell'app."""
    self = app

    outer = tk.Frame(self.tab_riduzione, bg=BG_MAIN)
    outer.pack(fill="both", expand=True, padx=10, pady=10)

    # Info header
    info_frame = tk.Frame(outer, bg=BG_CARD, padx=15, pady=10)
    info_frame.pack(fill="x", pady=(0, 10))
    tk.Label(info_frame, text="🔻 Pozioni di Riduzione",
             font=SECTION_FONT, bg=BG_CARD, fg=GOLD).pack(anchor="w")
    tk.Label(info_frame, text="Riduzione I: Rame | Riduzione II: Oro (upgrade da Rid I)",
    ...
    self.entry_rid_num = self.make_labeled_entry(prod_inner, "Numero pozioni:", "", row=0)

    tk.Label(prod_inner, text="Tipo pozione:", font=LABEL_FONT, bg=BG_CARD, fg=FG_TEXT
             ).grid(row=1, column=0, sticky="e", padx=(0, 8), pady=4)
    self.combo_rid_tipo = ttk.Combobox(prod_inner, values=["Riduzione I", "Riduzione II"],
    ...
    self.entry_rid_fungo = self.make_labeled_entry(price_inner, "Fungo marrone (1x):", "1.0", row=0)

    # Slimeball (readonly da Elisir)
    ...
    if 'slimeball' in self.icons:
        tk.Label(label_frame_slime, image=self.icons['slimeball'], bg=BG_CARD).pack(side="left", padx=(0, 4))
    tk.Label(label_frame_slime, text="Slimeball (1x):", font=LABEL_FONT, bg=BG_CARD, fg=FG_TEXT).pack(side="left")
    self.label_rid_slime = tk.Label(price_inner, text="-", font=LABEL_FONT, bg=BG_CARD, fg=ACCENT_LIGHT)
    ...
    self.label_rid_core = tk.Label(price_inner, text="-", font=LABEL_FONT, bg=BG_CARD, fg=ACCENT_LIGHT)
    ...
    self.label_rid_carbone = tk.Label(price_inner, text="-", font=LABEL_FONT, bg=BG_CARD, fg=ACCENT_LIGHT)
    ...
    self.label_rid_boccette = tk.Label(price_inner, text="-", font=LABEL_FONT, bg=BG_CARD, fg=ACCENT_LIGHT)
    ...
    self.entry_rid_prezzo = self.make_labeled_entry(sale_inner, "Prezzo vendita (b):", "", row=0, allow_empty=True)
    panel_sale.pack(padx=0, pady=(0, 8), fill="x")

    # === BOTTONE ===
    btn_container = tk.Frame(container, bg=BG_MAIN)
    btn_container.pack(pady=12)
    calc_btn = self.make_action_button(btn_container, "CALCOLA RIDUZIONE", self.do_calcola_riduzione, "success", "🧮")
    calc_btn.config(font=("Segoe UI", 12, "bold"), padx=24, pady=10)
    calc_btn.pack()

    # === RISULTATI ===
    self.make_result_area(col_out, "label_rid_preview", "text_rid_result")
